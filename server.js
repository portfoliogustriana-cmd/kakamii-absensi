const express = require('require'); // ketik const express = require('express'); jika di laptopmu express biasa
const { Pool } = require('pg');
const path = require('path');

const app = express();
const PORT = process.env.PORT || 8080; // Koyeb sangat menyukai port 8080 atau 3000
...
async function inisialisasiDatabase() {
    try {
        await pool.query(`CREATE TABLE IF NOT EXISTS absensi (
            id SERIAL PRIMARY KEY, nama TEXT, waktu TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
            tipe TEXT, lokasi TEXT, jarak TEXT, foto TEXT, status_waktu TEXT
        )`);
        await pool.query(`CREATE TABLE IF NOT EXISTS karyawan (
            id SERIAL PRIMARY KEY, username TEXT UNIQUE, password TEXT, pertanyaan TEXT, jawaban TEXT
        )`);
        await pool.query(`CREATE TABLE IF NOT EXISTS pengaturan (
            id INTEGER PRIMARY KEY, lat REAL, lon REAL, radius INTEGER, jam_masuk TEXT, jam_pulang TEXT, password_admin TEXT
        )`);
...
app.post('/login', async (req, res) => {
    ...
    try {
        if (userClean === 'admin') {
            ...
            if (password === config.rows[0].password_admin) return res.redirect('/admin');
            else return res.render('login', { error: 'Password Admin salah!', success: null });
        } else {
            const result = await pool.query("SELECT * FROM karyawan WHERE username = $1 AND password = $2", [userClean, password]);
            if (result.rowCount > 0) res.redirect('/absen?user=' + userClean); 
            else res.render('login', { error: 'Username atau Password salah!', success: null });
        }
...
app.post('/register', async (req, res) => {
    ...
    try {
        await pool.query("INSERT INTO karyawan (username, password, pertanyaan, jawaban) VALUES ($1, $2, $3, $4)", [userClean, password, pertanyaan, jawaban.toLowerCase().trim()]);
        res.redirect('/?success=Akun berhasil dibuat! Silakan login.');
    } catch (err) { res.render('register', { error: 'Username sudah terdaftar! Pilih nama lain.' }); }
});
...
app.get('/absen', (req, res) => { res.render('absen', { user: req.query.user || 'Karyawan', error: req.query.error || null, success: req.query.success || null }); });
...
app.post('/absen', async (req, res) => {
    ...
    try {
        const cekAbsen = await pool.query(`SELECT id FROM absensi WHERE nama = $1 AND tipe = $2 AND waktu::date = CURRENT_DATE`, [nama, tipe]);
        ...
        await pool.query("INSERT INTO absensi (nama, tipe, lokasi, jarak, foto, status_waktu) VALUES ($1, $2, $3, $4, $5, $6)", [nama, tipe, `${lat},${lon}`, `${Math.round(jarakMeter)} meter`, fotoBase64, statusWaktu]);
...
app.get('/admin', async (req, res) => {
    const error = req.query.error || null; const success = req.query.success || null;
    try {
        ...
        const configResult = await pool.query("SELECT * FROM pengaturan WHERE id = 1");
        res.render('admin', { riwayat: absensiResult.rows, karyawan: karyawanResult.rows, config: configResult.rows[0], error, success });
...
app.get('/admin/karyawan/hapus/:id', async (req, res) => {
    try { await pool.query("DELETE FROM karyawan WHERE id = $1", [req.params.id]); res.redirect('/admin'); } 
    catch (err) { res.status(500).send("Gagal menghapus"); }
});

app.post('/admin/karyawan/edit', async (req, res) => {
    const { id, username, password } = req.body;
    try { await pool.query("UPDATE karyawan SET username = $1, password = $2 WHERE id = $3", [username.toLowerCase(), password, id]); res.redirect('/admin'); } 
...
app.post('/admin/pengaturan', async (req, res) => {
    ...
    try {
        await pool.query("UPDATE pengaturan SET lat = $1, lon = $2, radius = $3, jam_masuk = $4, jam_pulang = $5 WHERE id = 1", [lat, lon, radius, jam_masuk, jam_pulang]);
...
app.post('/admin/ganti-password-admin', async (req, res) => {
    ...
    try {
        const config = await pool.query("SELECT password_admin FROM pengaturan WHERE id = 1");
        if (password_lama !== config.rows[0].password_admin) return res.redirect('/admin?error=Gagal ganti password! Password lama yang kamu masukkan salah.');
...
app.get('/admin/download', async (req, res) => {
    try {
        const result = await pool.query(`SELECT id, nama, tipe, to_char(waktu AT TIME ZONE 'Asia/Jakarta', 'YYYY-MM-DD') as tgl, to_char(waktu AT TIME ZONE 'Asia/Jakarta', 'HH24:MI:SS') as jam, jarak, status_waktu FROM absensi ORDER BY nama ASC, waktu ASC`);
